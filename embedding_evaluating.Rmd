...
## Filter subjects to compare with MOMENT - IGNORE

```{r eval=FALSE}
# get matching subjects as MOMENT
data_gpt1536 = data_gpt1536_entropy |> 
  filter(seqn %in% data_moment_50$seqn) 

data_gpt50_train = data_gpt50_entropy |> 
  filter(seqn %in% data_moment50_train$seqn) 

data_bert768_train = data_bert768_entropy |> 
  filter(seqn %in% data_moment50_train$seqn) 

data_bert50_train = data_bert50_entropy |> 
  filter(seqn %in% data_moment50_train$seqn)  

data_cohere1024_train = data_cohere1024_entropy |> 
  filter(seqn %in% data_moment50_train$seqn)  

data_cohere50_train = data_cohere50_entropy |> 
  filter(seqn %in% data_moment50_train$seqn)  
```
...
split_moment = sample(nrow(data_moment_50), floor(nrow(data_moment_50)*0.8), replace = FALSE)
train_m = data_moment_50[split_moment,]
test_m = data_moment_50[-split_moment,]
...
auc_EntroGPT50 = model_auc(data_gpt50, sim = 10)
auc_EntroBERT768 = model_auc(data_bert768, sim = 10)
auc_EntroBERT50 = model_auc(data_bert50, sim = 10)
auc_EntroCohere1024 = model_auc(data_cohere1024, sim = 10)
auc_EntroCohere50 = model_auc(data_cohere50, sim = 10)
...
dim(auc_base)
dim(auc_EntroGPT50)
dim(auc_EntroBERT768)
dim(auc_EntroBERT50)
dim(auc_EntroCohere1024)
dim(auc_EntroCohere50)
dim(auc_MOMENT1024)
dim(auc_MOMENT50)
